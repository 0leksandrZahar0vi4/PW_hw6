...
for group_id in range(1, 4):
    for _ in range(50):
        cur.execute("INSERT INTO users (name, groups_id) VALUES (%s, %s) RETURNING id",
                    (fake.name(), group_id))
        users_id = cur.fetchone()[0]
        for subject_id in range(1, 7):
            for _ in range(3):
                cur.execute("INSERT INTO grades (users_id, subject_id, grade, grade_date) VALUES (%s, %s, %s, %s)",
                            (users_id, subject_id, random.randint(60, 101), fake.date_this_decade(), ))
